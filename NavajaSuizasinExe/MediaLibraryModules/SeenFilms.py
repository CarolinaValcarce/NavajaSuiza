...
class SeenFilms():
    ...
    def __init__(self,master):
        ...
        self.master=master
        master.title('PELICULAS VISTAS DE LA HEMEROTECA:')
        master.geometry('550x525+500+5')
        ...
        self.l_film= Label (master,text='Introduce codigo de la PELICULA:',bg='light slate grey', fg='white').place(x=50, y=80)
        self.e_film= Entry (master, width=20)
        self.e_film.place(x=350, y=80)
        ...
        self.b_selectiondate=Button(master,text='Confirma fecha de vision',bd=5,command=self.write_date).place (x=50,y=250)
    ...
    def save(self):
        ...
        try:
            ...
            #cursor.execute ('DROP TABLE PeliculasVistas')
            cursor.execute('''CREATE TABLE IF NOT EXISTS PeliculasVistas (DNI_Cliente INTEGER,
                ...
                FOREIGN KEY(Codigo_Peliculas) REFERENCES Peliculas(Codigo) ON UPDATE CASCADE ON DELETE CASCADE)''')

            if ((datetime.strptime(self.calendar.get_date(),'%m/%d/%y').date())<=date.today()):   
            
                cursor.execute ('''INSERT INTO PeliculasVistas ( DNI_Cliente,
                            Codigo_Peliculas, Puntuacion, Fecha) VALUES (?,?,?,?)''',
                           (self.e_customer.get(),self.e_film.get(), self.score.get(), self.calendar.get_date()))

                self.e_customer.delete(0,END)
                self.e_film.delete(0,END)
            ...
            showerror(title='SE HA PRODUCIDO UN  ERROR', message='Ese cliente con esa misma pelicula ya han  sido introducidos. '+'\n'+
            'O bien, estas introduciendo un cliente o una pelicula inexistentes')
    ...
    def show(self):
        ...
        try:
            ...
            cursor.execute('PRAGMA foreign_keys=ON')
            cursor.execute ('SELECT * FROM PeliculasVistas')
            ...
            showwarning(title='ATENCION', message='Todavia no has introducido la preferencia de ningun cliente por nnguna pelicula')

    def get_score(self):
        '''Rellenamos el campo de entrada de puntuacion automaticamente con la seleccion de puntuacion recogida al presionar el radiobutton el usuario.'''
        self.e_score.delete(0,END)
